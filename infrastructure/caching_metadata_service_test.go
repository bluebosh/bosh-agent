...
func describeCachingMetadataService() {
	...
	Describe("GetInstanceID", func() {
		...
		Context("when the instance ID path is not present", func() {
			BeforeEach(func() {
				instanceIDPath = ""
				httpMetadataService = NewHTTPMetadataServiceInstance(ts.URL, metadataHeaders, "/user-data", instanceIDPath, sshKeysPath, dnsResolver, platform, logger)
				metadataService = NewCachingMetadataService("/fake-user-data-cache-path", dnsResolver, fs, logger, httpMetadataService)
			})
	...
	Describe("GetRegistryEndpoint", func() {
		...
		BeforeEach(func() {
			...
			registryURL = &url
			dnsServer = nil
			httpRegistrylocalCacheEnabled := false
			userdataCache = &httpRegistrylocalCacheEnabled
			httpRegistryCachePath := "/fake-user-data-cache-path"
			userdataCachePath = &httpRegistryCachePath
			// Encoded content: {"registry":{"endpoint":"http://fake-registry-ip-from-cache"}}
			userdataCacheContent = "{\"registry\":{\"endpoint\":\"http://fake-registry-ip-from-cache\"}}"
			registryContentEncoded = "\"eyJyZWdpc3RyeSI6eyJlbmRwb2ludCI6Imh0dHA6Ly9mYWtlLXJlZ2lzdHJ5LWlwLWZyb20tY2FjaGUifX0\""
		...
		Context("when metadata does not contain dns servers", func() {
			...
			Context("when enabling local cache", func() {
				...
				It("returns error because cache file could not be wrote", func() {
	...
	Describe("Retryable Metadata Service Request", func() {
		...
		Context("when server returns an HTTP Response with status code !=2xx (as defined by the request retryable) more than 10 times", func() {
			It("returns an error containing the HTTP Response", func() {
				...
				Expect(err).To(MatchError(fmt.Sprintf("Getting user data from remote server: invalid status from url %s/user-data: 500", ts.URL)))
			})
		})
	})
