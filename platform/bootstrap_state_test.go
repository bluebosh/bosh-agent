...
var _ = Describe("State", func() {
	...
	Describe("SaveState", func() {
		...
		It("saves the state file with the ephemeral_partitioned passed in", func() {
			...
			Expect(readerr).ToNot(HaveOccurred())
			Expect(string(contents)).To(Equal(`{"Linux":{"hosts_configured":true,"ephemeral_partitioned":true}}`))
		})
