...
var _ = Describe("NewLinuxDiskManager", func() {
	...
	Context("when partitioner type is not set", func() {
		It("returns disk manager configured to use sfdisk", func() {
			...
			diskManager := disk.NewLinuxDiskManager(logger, runner, fs, opts)
			Expect(diskManager.GetEphemeralDevicePartitioner()).To(Equal(disk.NewEphemeralDevicePartitioner(disk.NewPartedPartitioner(logger, runner, clock.NewClock()), disk.NewUtil(runner, mounter, fs, logger), logger, runner, fs, clock.NewClock())))
		})
	})
