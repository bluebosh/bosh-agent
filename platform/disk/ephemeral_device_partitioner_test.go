...
var _ = Describe("EphemeralDevicePartitioner", func() {
	...
	BeforeEach(func() {
		...
		partitioner = NewEphemeralDevicePartitioner(partedPartitioner, diskUtil, logger, fakeCmdRunner, fakefs, fakeclock, "/setting/path.json")
	})
...
`},
			...
			It("creates partitions using parted starting at the 1048576 byte", func() {
				...
				Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-m", "/dev/edx", "unit", "B", "print"}))
				Expect(scrubbedCommands).To(ContainElement([]string{"blkid"}))
				Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-0", "1048576", "8590983167"}))
				Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-1", "8590983168", "17180917759"}))
				Expect(scrubbedCommands).To(ContainElement([]string{"partprobe", "/dev/edx"}))
				Expect(scrubbedCommands).To(ContainElement([]string{"udevadm", "settle"}))

				Expect(fakefs.ReadFileWithOptsCallCount).To(Equal(1))
			})
		})
...
`},
				...
				It("recreates partitions using parted starting at the 1048576 byte", func() {
					...
					scrubbedCommands := scrubPartitionNames(fakeCmdRunner.RunCommands)
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-m", "/dev/edx", "unit", "B", "print"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "mklabel", "gpt"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"blkid"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"wipefs", "-a", "/dev/edx1"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-0", "1048576", "17180917759"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-1", "17180917760", "25770852351"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"partprobe", "/dev/edx"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"udevadm", "settle"}))
...
`},
				...
				It("recreates partitions using parted starting at the 1048576 byte", func() {
					...
					Expect(scrubbedCommands).To(ContainElement([]string{"blkid"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"wipefs", "-a", "/dev/edx1"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"wipefs", "-a", "/dev/edx2"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "/dev/edx", "rm", "1"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "/dev/edx", "rm", "2"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-0", "1048576", "8590983167"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"parted", "-s", "/dev/edx", "unit", "B", "mkpart", "fake-agent-id-1", "8590983168", "17180917759"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"partprobe", "/dev/edx"}))
					Expect(scrubbedCommands).To(ContainElement([]string{"udevadm", "settle"}))
		...
		Context("when getting agent ID returns an error", func() {
			BeforeEach(func() {
				fakefs.RegisterReadFileError("/setting/path.json", errors.New("fake-read-file-error"))
			})
			...
			It("throw an error", func() {
				...
				Expect(len(fakeCmdRunner.RunCommands)).To(Equal(0))
				Expect(err.Error()).To(ContainSubstring("Getting agent ID"))
