...
func (net UbuntuNetManager) writeNetworkInterfaces(
	...
	err = net.fs.Walk(filepath.Join(systemdNetworkFolder), func(filename string, _ os.FileInfo, err error) error {
		if err != nil {
			return err
		}
		if strings.Contains(filename, ":") {
			err := mergeVirtualInterfaceNetworkFile(filename)
			if err != nil {
				return err
			}
		}
...
// merge virtual interface to its physical interface config file
func mergeVirtualInterfaceNetworkFile(filename string) (err error) {
	...
	addrRegexp := regexp.MustCompile(`Address=.*`)
	name := strings.Split(nameRegexp.FindString(string(buf)), "=")[1]
	labelLine := "Label=" + name
	addressLine := addrRegexp.FindString(string(buf))
	appendSection := "\n[Address]\n" + labelLine + "\n" + addressLine + "\n"

	physicalInterfaceNetworkFile := interfaceConfigurationFile(strings.Split(name, ":")[0])
	_, err = os.Stat(physicalInterfaceNetworkFile)
	if os.IsNotExist(err) {
		return err
	}
	f, err := os.OpenFile(physicalInterfaceNetworkFile, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return err
	}
	defer f.Close()
	if _, err := f.WriteString(appendSection); err != nil {
		return err
	}
	return nil
}
