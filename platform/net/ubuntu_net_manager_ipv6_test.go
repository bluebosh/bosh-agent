	...
	Describe("SetupNetworking", func() {
		...
		It("writes /etc/network/interfaces with static inet6 configuration when manual network is used", func() {
			...
			Expect(matches).To(ConsistOf(
				"/etc/systemd/network/00_ethstatic1.network",
				"/etc/systemd/network/00_ethstatic2.network",
				"/etc/systemd/network/00_ethstatic3.network",
			))
			networkConfig := fs.GetFileTestStat("/etc/systemd/network/00_ethstatic1.network")
...
`))
			networkConfig = fs.GetFileTestStat("/etc/systemd/network/00_ethstatic2.network")
			Expect(networkConfig).ToNot(BeNil())
			Expect(networkConfig.StringContents()).To(Equal(`# Generated by bosh-agent
[Match]
Name=ethstatic2
...
`))
			networkConfig = fs.GetFileTestStat("/etc/systemd/network/00_ethstatic3.network")
			Expect(networkConfig).ToNot(BeNil())
			Expect(networkConfig.StringContents()).To(Equal(`# Generated by bosh-agent
[Match]
Name=ethstatic3
